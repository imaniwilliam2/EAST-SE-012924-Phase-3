class Hotel:
    ...
    # Review Method
    def reviews(self):
        return [review for review in Review.all if review.hotel == self]
...
class Customer:
    ...
    def first_name(self, first_name_parameter):
        if(not hasattr(self, 'first_name')) and (isinstance(first_name_parameter, str)) and (len(first_name_parameter) >  0):
            ...
            raise Exception("First name cannot be change and must be a string greater than 0 characters long!")
    ...
    def last_name(self, last_name_parameter):
        if(not hasattr(self, 'last_name')) and (isinstance(last_name_parameter, str)) and (len(last_name_parameter) > 0 ):
            ...
            raise Exception("Last name cannot be change and must be a string greater than 0 characters long!")
...
class Review:
    ...
    def text(self, text_parameter):
        if(not hasattr(self, 'text')) and (isinstance(text_parameter, str)) and (3 <= len(text_parameter) <= 40):
            ...
            raise Exception("Text cannot be changed and must be a string between 3 and 40 characters!")
        
    def __repr__(self):
        return f"<Review #{self.id} => Rating => {self.rating}, Text => {self.text}.>"
